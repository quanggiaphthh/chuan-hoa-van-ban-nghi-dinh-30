...
t=root/'v22-runtime/tests/LegalValidator.Tests/V26FinalAuthorityRevalidationTests.cs'; z=t.read_text(); z=z.replace('Assert.Null(Type.GetType("Nd30.LegalValidator.Execution.PlanOperation, LegalValidator")?.GetConstructors().FirstOrDefault());','Assert.False(Type.GetType("Nd30.LegalValidator.Execution.PlanOperation, LegalValidator")!.IsPublic);')
t.write_text(z)
print('V26 final namespace + constructor + exact intent binding correctives applied')
